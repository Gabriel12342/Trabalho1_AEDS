...
## Limitações
- Tamanho de população pequeno: Com apenas `m = 20` indivíduos, a população perde diversidade genética rapidamente, favorecendo a convergência prematura observada na Fase 2. Populações maiores sustentariam a exploração do espaço de busca por mais tempo.
- Algoritmo de ordenação ineficiente: O uso de Selection Sort (O(m²)) é adequado para populações pequenas, mas se tornaria um gargalo relevante com o crescimento de m. Para escalabilidade, seria necessário substituí-lo por um algoritmo mais eficiente.
- Elitismo parcial sem proteção explícita: Os `m/2` melhores indivíduos sobrevivem a cada geração (elitismo implícito via ordenação + substituição apenas da segunda metade). No entanto, apenas populacao[0] está protegido da mutação — os demais membros da elite permanecem vulneráveis a perturbações, o que pode degradar boas soluções parciais sem garantia de recuperação.
- Taxa de mutação fixa: O delta de mutação `∈ [-1, 1]` e a probabilidade de 50% são constantes ao longo de toda a execução. Nas gerações finais, quando o erro já é muito pequeno, esse delta pode ser grande demais para um refinamento preciso.
...
- Restrito à regressão linear: A implementação está hardcoded para o modelo `y = ax + b`. Não é possível ajustar outros tipos de função sem modificar diretamente o código-fonte.
- Dataset de validação pequeno e sem ruído: O experimento utilizou apenas `n = 8` pontos gerados por uma reta perfeita. Em cenários reais, com dados ruidosos e conjuntos maiores, o comportamento do algoritmo pode ser substancialmente diferente.
---
## Conclusão:

Conclusão
O Algoritmo Genético implementado demonstrou ser eficaz para o problema de ajuste de uma função linear `y = ax + b` a um conjunto de pontos. Com apenas 500 gerações e uma população de 20 indivíduos, foi possível convergir para uma solução muito próxima do ótimo teórico — atingindo `a = 1.990` e `b = 1.046` frente aos valores ideais `a = 2.0` e `b = 1.0`, com erros relativos de 0,5% e 4,6% respectivamente e um MSE final de apenas 0,00053.
